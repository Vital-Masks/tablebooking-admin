...
interface AddonRevenueCardProps {
  title: string;
  revenue: number;
...
const AddonRevenueCard: React.FC<AddonRevenueCardProps> = ({
  title,
  revenue,
  noOfRestaurants,
  trend,
  ...
  return (
    <article 
      ...
      aria-label={`${title} add-on revenue card`}
    >
      {/* Header with title and icon */}
      <div className="relative flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-medium text-white mb-2">{title} (LKR)</h3>
        ...
        {/* Icon container - top right (money bag) */}
        <div 
      ...
      <div className="mb-4">
        <div className="flex justify-between items-center text-sm">
          <span className="text-gray-300">No of Restaurants:</span>
          <span className="font-semibold text-white">{noOfRestaurants.toLocaleString()}</span>
        </div>
      </div>
