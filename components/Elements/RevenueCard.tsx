...
const RevenueCard: React.FC<RevenueCardProps> = ({
  ...
  return (
    <article 
      ...
      aria-label={`${title} revenue card`}
    >
      {/* Header with title and icon */}
      <div className="relative flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-medium text-white mb-2">{title}</h3>
          <p className="text-3xl font-bold text-white leading-tight" aria-label={`Revenue: ${formatCurrency(revenue)} LKR`}>
            {formatCurrency(revenue)} <span className="text-lg font-normal">LKR</span>
          </p>
        </div>
