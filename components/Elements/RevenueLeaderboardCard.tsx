...
interface RevenueLeaderboardCardProps {
  rank: number;
...
const RevenueLeaderboardCard: React.FC<RevenueLeaderboardCardProps> = ({
  rank,
  restaurantName,
  revenue,
  positionChange,
  ...
  return (
    <article 
      className="relative overflow-hidden rounded-lg bg-gray-700 p-6 border border-gray-600 text-white shadow-sm min-h-[220px] flex flex-col"
      aria-label={`Rank ${rank} in revenue: ${restaurantName}`}
    >
      ...
      <div className="relative flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-gray-300">in Revenue</span>
          </div>
      ...
      {/* Revenue */}
      <div className="mb-4">
        <p className="text-2xl font-bold text-white leading-tight" aria-label={`Revenue: ${formatCurrency(revenue)} LKR`}>
          {formatCurrency(revenue)} <span className="text-base font-normal">LKR</span>
        </p>
      </div>
...
export default RevenueLeaderboardCard;
